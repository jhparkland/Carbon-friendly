...
class Check_GPU:
    def __init__(self, model_name, gpu_id, voltage_rate) -> None:
        ...
        # self.prof = profiler._KinetoProfile(activities=[profiler.ProfilerActivity.CPU, profiler.ProfilerActivity.CUDA], profile_memory=True, record_shapes=True)
        ...
        if self.under_volting_rate != None:
            os.makedirs(f'/VGGNet_Imagenet/Experiment/exp{int(self.under_volting_rate*100)}', exist_ok=True)
        else:
            os.makedirs(f'/VGGNet_Imagenet/Experiment/exp', exist_ok=True)
    ...
    def save_model_firebase(self, model_path):
        ...
        self.storage.child(self.zone+"/"+filename).put(model_path)                    
        self.db.child('optim').child('request').set(0)
    ...
    def load_model_from_firebase(self):
        ...
        model_path = "/Experiment/exp/" + self.dl_model + "latest_model.pt"
        file_name = self.dl_model + "_" + str(i) + '_iter_model.pt'
        self.storage.child(zone+"/"+file_name).download(model_path)
    ...
    def get_gpu_info(self):
        try:
            # nvidia-smi 명령 실행
            command = f"nvidia-smi --id {self.gpu_id} --query-gpu=name,memory.total --format=csv,noheader,nounits"
            result = subprocess.check_output(command, shell=True, universal_newlines=True)
    ...
    def get_min_max_freq_info(self):
        try:
            ...
            command = f"nvidia-smi --id {self.gpu_id} -q -d SUPPORTED_CLOCKS"
            result = subprocess.check_output(command, shell=True, universal_newlines=True)
    ...
    def set_gpu_core_clock(self, core_clock_freq):
        try:
            # persistent mode on
            command = f"nvidia-smi --id {self.gpu_id} -pm=1"
            subprocess.run(command, shell=True, check=True)

            # setting command for change gpu core clock
            command = f"nvidia-smi --id {self.gpu_id} --lock-gpu-clocks={core_clock_freq},{core_clock_freq}"
            # nvidia-smi 명령 실행
            subprocess.run(command, shell=True, check=True)
    ...
    def save_csv(self):
        ...
        if self.under_volting_rate != None:
            save_path = f"/VGGNet_Imagenet/Experiment/exp{int(self.under_volting_rate * 100)}/{int(self.under_volting_rate * 100)}_measurement.csv"
        else:
            save_path = f"/VGGNet_Imagenet/Experiment/exp/optim_measurement.csv"
    ...
    def cost_func(self, f):
        ...
        print("time", sum(self.cost_weight['execution_time'] * time_cost))
        print("power", sum(self.cost_weight['power_usage'] * power_cost))
        print("scaling", sum(self.cost_weight['freq_scaling'] * scaling_cost))
